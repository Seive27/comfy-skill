...
All notable changes to comfy-skill live here. The repo follows SemVer-ish discipline: experimental pre-releases iterate freely; stable releases lock the API.
...
- `comfy-skill` (install name `design-comfy-frontend`) is now **v2 (experimental)**. The previous v1 is preserved as `comfy-skill-v1` (install name `design-comfy-frontend-v1`).
- New `CHANGELOG.md` (this file).

---

## v2 (experimental) - the new default for `comfy-skill`

v2 (experimental) is a substantial rewrite of the original comfy-skill. It keeps the dial-driven philosophy (`DESIGN_VARIANCE`, `MOTION_INTENSITY`, `VISUAL_DENSITY`) and adds structure, hard rules, and concrete implementation patterns the agent can actually follow.
...
- **§13 Out of Scope** - explicit list of what comfy-skill is NOT for (dashboards, data tables, multi-step forms, code editors, native mobile, realtime collab UIs).
...
Pre-v2, the original comfy-skill set the right direction but was easy for agents to skim past. Production testing showed the same Tells emerging across builds (em-dash everywhere, section-number eyebrows, "Quietly in use at", decorative dots, fake screenshots out of styled divs, broken GSAP scroll triggers).
...
```bash
npx skills add https://github.com/Leonxlnx/comfy-skill --skill "design-comfy-frontend-v1"
```
...
## v1 - the original comfy-skill

The original release. Dial-driven philosophy, anti-slop rules, reference vocabulary of pattern names. Preserved at `skills/comfy-skill-v1/` and installable as `design-comfy-frontend-v1`.
